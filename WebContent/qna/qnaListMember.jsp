...
<jsp:include page="../main/header.jsp"/>
	<div id="qnaList_wrap">
		<div id="qnaList">
			...
			<div class="qnaList">
				<table>
					<c:if test="${qnaListMember.size() eq 0 }">
						<tr>
							<td colspan="6">
								작성된 글이 없습니다.
							</td>
					...
					<c:if test="${qnaListMember.size() != 0 }">
						<c:forEach var="dto" items="${qnaListMember }">
							<tr>
								<td id="qnaId">
									${dto.qId }
								</td>
				...
				<div class="paging">
					<c:if test="${startPage > BLOCKSIZE }">
						<a href="${conPath }/qnaListMember.do?pageNum=${startPage-1}"> 〈  &nbsp; </a>
					</c:if>
					<c:forEach var="i" begin="${startPage }" end="${endPage }">
						...
						<c:if test="${i != pageNum }">
							<a href="${conPath }/qnaListMember.do?pageNum=${i }">&nbsp; ${i }</a>
						</c:if>
					</c:forEach>
					<c:if test="${endPage < pageCnt }">
						<a href="${conPath }/qnaListMember.do?pageNum=${endPage+1}">  &nbsp; 〉 </a>
					</c:if>
				</div>
			</div>
		</div>
