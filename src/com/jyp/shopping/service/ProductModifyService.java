...
public class ProductModifyService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		// 상품 정보 수정 시 사진 파일을 새로 첨부 안하면, 원래 사진 파일로 
		String dbpImage1 = mRequest.getParameter("dbpImage1");
		String dbpImage2 = mRequest.getParameter("dbpImage2");
		String pImage1 = pImage[1]!=null? pImage[1]:dbpImage1;
		String pImage2 = pImage[0]!=null? pImage[0]:dbpImage2;
		
		ProductDao dao = ProductDao.getInstance();
		ProductDto dto = new ProductDto(pId, pCategory, pName, pPrice, pDiscount, pImage1, pImage2, pContent, pPolicy1, pPolicy2, pStock, null, 0);
		int result = dao.productModify(dto);
		if(result==ProductDao.SUCCESS) {
			/* request.setAttribute("product", dto); */
			request.setAttribute("productInsertMsg", "상품 정보 수정을 성공했습니다.");
		}else {
			request.setAttribute("productInsertMsg", "상품 정보 수정을 실패했습니다."+dto.toString());
		}
